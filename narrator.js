/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Miraat
...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','timeline','causes','solutions','lessons'],
    tabIndex: 0,
  ...
  function extractCards(panel) {
    ...
    if (tabName !== 'about') {
      ...
      const desc = panel.querySelector('.page-subtitle');
      if (title) {
        cards.push({
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.timeline-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'timeline' });
      });
    } else if (tabName === 'causes') {
      panel.querySelectorAll('.cause-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'cause' });
      });
    } else if (tabName === 'solutions') {
      panel.querySelectorAll('.solution-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'solution' });
      });
  ...
  function switchToTab(tabName) {
    ...
    if (tabBtn) {
      ...
      setTimeout(function() {
        ...
        const title = getActivePanel().querySelector('.page-title');
        if (title) {
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'المرآة' : l === 'fr' ? 'Le Miroir' : 'The Mirror',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('miraat-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('miraat-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('miraat-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('miraat-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('miraat-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('miraat-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('miraat-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('miraat-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('miraat-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('miraat-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      '.about-card', '.disclaimer-card',
      '.timeline-card', '.cause-card', '.solution-card', '.lesson-card'
    ];
  ...
  var origRenderQuiz = window.renderQuiz;
  if (origRenderQuiz) {
    window.renderQuiz = function() { origRenderQuiz(); setTimeout(injectSpeakButtons, 100); };
  }
